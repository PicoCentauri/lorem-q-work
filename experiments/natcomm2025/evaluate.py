...
# One split: the 90/10 group-wise split made by prepare.py. There is no test
# set -- the source ships none, and carving one out of the same trajectories
...
def load_frames(name, polarizable_only, n=None):
    """Read the converted xyz and keep the validation groups.

    Nothing is converted here: total_charge and work_function were derived in
    ../../datasets/natcomm2025/convert.py. `polarizable_only` is ignored --
    this dataset has no such flag, and every frame is marked polarizable=True
    so the machinery inherited from razor collapses to a single subset.

    The split is reconstructed with the same seed and group logic as
    prepare.py, so `valid` here is exactly the frames the model did not train
    on. Splitting on `group` matters: the 4-18 charge states of one geometry
    share atomic positions.
    """
    ...
    for atoms in frames:
        ...
        mf = float(np.abs(atoms.get_forces()).max())
...
def plot_rmse_vs_charge(all_rows, split, name):
    ...
    fig.suptitle(f"natcomm2025 -- {split} -- RMSE vs charge ({hatched})")
...
def plot_split(all_rows, split, name):
    ...
    fig.suptitle(f"natcomm2025 -- {split}")
    Path("figures").mkdir(exist_ok=True)
    fig.savefig(Path("figures") / name, transparent=True, bbox_inches="tight")
    plt.close(fig)
    print(f"saved figures/{name}")
